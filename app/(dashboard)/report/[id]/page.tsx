...
import { useMyTicket, useSendTicketMessage } from "@/hooks/useSupportTickets";
...
export default function ReportThreadPage() {
  ...
  const { data, isLoading } = useMyTicket(ticketId);
  const { mutate: sendMessage, isPending } = useSendTicketMessage(ticketId);
    ...
    <div className="flex flex-col gap-6 h-[calc(100vh-8rem)]">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <Link href="/report" className="text-muted-foreground hover:text-foreground shrink-0">
            <ArrowLeft size={18} />
          </Link>
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{CATEGORY_CONFIG[ticket.category].label}</p>
          </div>
        </div>
        <span
          className={cn(
          ...
          <Textarea
            ...
            placeholder={
              ticket.status === "resolved" || ticket.status === "closed"
                ? "Replying will reopen this report..."
                : "Type a message..."
            }
            rows={1}
