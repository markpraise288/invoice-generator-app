...
export default function ResetPasswordPage() {
  ...
  const showCodeError = (touched.code || attempted) && !codeValid;

  const passwordsMatch = confirmPassword.length > 0 && confirmPassword === password;
  const showMatchError = (touched.confirmPassword || attempted) && confirmPassword.length > 0 && !passwordsMatch;

  const canSubmit = codeValid && passwordValid && passwordsMatch;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("error");
      setMessage(err?.message || "That code didn't work. Check it and try again.");
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      // stays silent — the original request already told them a code was sent
    } finally {
      setResending(false);
    }
  };
              ...
              {status === "success" ? (
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {showCodeError && (
                      <p className="text-xs text-red-500">Enter the 6-digit code from your email.</p>
                    )}
                    <button
                      type="button"
