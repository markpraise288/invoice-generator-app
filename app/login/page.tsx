...
import { AlertCircle, ArrowLeft, Workflow, X } from "lucide-react";
...
export default function LoginPage() {
  ...
  const loginHandler = async (formData: FormDataType) => {
    ...
    try {
      ...
      setError(err?.message || "We couldn't log you in. Check your email and password and try again.");
    }
  };
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-white dark:bg-gray-950">
      ...
      <div className="hidden lg:flex flex-col justify-between bg-linear-to-br from-indigo-700 via-indigo-600 to-teal-600 p-12 text-white relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="mt-4 text-indigo-100 max-w-sm">
            Your pipeline, invoices, and finance reports are exactly where you left them.
          </p>
        ...
        <p className="relative text-xs text-indigo-200">
          Trouble signing in? Email support@businessflow.com
        </p>
      </div>
      ...
      <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-950 px-4 py-8">
        ...
        <div className="flex-1 flex items-center justify-center py-8">
          <AuthForm title="Login" submitRequest={loginHandler} embedded />
        </div>

        <p className="text-center text-xs text-gray-400 dark:text-gray-500">
          Need an account?{" "}
          <Link href="/signup" className="underline hover:text-gray-600 dark:hover:text-gray-300">
            Sign up free
          </Link>
        </p>
      </div>
